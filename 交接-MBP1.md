...
- 2026-08-08 视觉重塑为「暖纸矿彩」：一张 720×1279、16KB 的平整细纸 WebP 统一大厅与弹层，
  只在边缘留下极淡花叶和星盘痕迹；明确不做褶皱、破边、旧污渍与泛黄相框。棋盘改成暖胡桃矿盘，
...
- 浏览器 390×844 实机视口已验大厅、行囊、闯关榜、聚会台、交接、棋盘与实时战绩层；
  `window.__selftest()` 为 **133/133**。
- **CI 两道门全自动**：合并 main = 发版（Pages + EdgeOne 直传 + 冒烟）。
  `gemfall-verify.yml` 里含 `node leaderboard/pacer.test.mjs`（配速员 14 条不变量）。
...
- **改配速员任何字段前**：`leaderboard/pacer.test.mjs` 顶部注释（14 条不变量，
