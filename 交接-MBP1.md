...
- **《灵石远征 GEMFALL》减法阶段**：单文件 `match/index.html`（约 545KB），
...
- 视觉升级为「夜曜琉金」：墨蓝黑、暖金与少量紫蓝矿彩统一大厅、棋盘、按钮和弹层；没有新增位图，
  移除 `backdrop-filter` 与全部 CSS 无限循环动画，保留一次性反馈和 reduced-motion 总让路。主资源仍 54 张。
- 浏览器 390×844 实机视口已验大厅、聚会台、交接、棋盘与实时战绩层；`window.__selftest()` 为 **133/133**。
