...
卷子上印品牌短址 `https://myskme.com/q/S2E5`，由边缘函数跳到当前真正的题库地址。

**为什么要这一层**：纸是永久的，链接不是。题库页面将来可能搬到自有域名、换目录，
而已经印出去的卷子改不了。有了这一跳，**只要改一个常量，所有卷子上的码就继续有效**——
以后不必再在「保住旧链接」和「往前走」之间做取舍。

- 源码：`deploy/homepage/edge-functions/q/`（`[[default]].js` 带码跳转，`index.js` 无码回书架）。
- 改去向只改 `GAME_BASE` / `SHELF` 两个常量，别把新地址散落到别处。
- 兑换码限定 `[A-Za-z0-9_-]{1,32}`，顺带挡掉开放跳转；码不合法就回书架，不甩 404 给学生。
- 线上验收有两条盯着它（带码跳转、无码回书架）——这一跳断了等于学生手里的码全作废。

## 发布：人工确认，纯题库数据除外

| 改了什么 | 会发生什么 |
|---|---|
| 纯 `banks/` 数据 | **自动构建 + 自动发布生产环境** |
| 其他任何文件（哪怕同时也改了 banks/） | 只构建、只校验，**必须人工确认才发布** |

**为什么给数据开这条窄道**：题库数据变更后如果不自动上线，`myskme.com` 上的书架就会比
GitHub Pages 那份旧——学生扫新卷子上的兑换码会找不到题库，正是以前出过的
「卷上兑换码线上 404」。而出题是高频动作，每次都要人去点一次发布不现实。

判定在 build 任务的「判断本次推送是否只动了题库数据」一步：对比本次推送的前后提交，
**所有变更文件都在 `banks/` 下**才算数据变更。掺了任何其他文件一律回到人工确认，
闸门不动。
