...
async function verify() {
  ...
  assert(html.includes('20260812.6-monkey-100-party-pwa'), '线上仍是多人桌面版之前旧版本');
  ...
  const swResponse=await fetch('https://monkey.myskme.com/sw.js?release-check='+stamp,{signal:AbortSignal.timeout(15000)});const sw=await swResponse.text();assert(swResponse.status===200&&sw.includes('myskme-monkey-20260812-6'),'线上离线缓存脚本不匹配');
