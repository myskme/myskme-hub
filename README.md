...
- 一屏式作品启动台：页面本身不滚动；桌面使用「分类栏 + 作品格 + 详情舞台」，手机自动切换为「详情舞台 + 四格分页 + 底部分区」。
- 精选 / 游戏 / 学习 / 世界四个入口，支持全局搜索、分页与键盘方向键；自动记住上次分区、页码、选中作品和最近打开记录。
- 点作品格只切换详情，不会误跳；在详情舞台中再明确选择启动、扫码、复制或相关支线入口。
- 浅色 / 深色主题默认跟随系统，主题、打印、分享和更多工具统一收进顶部操作区。
...
- 所有模式都要验收 `390×844` 与 `320×568`，同时检查安全区域、横向溢出、触控尺寸和动态视口。
