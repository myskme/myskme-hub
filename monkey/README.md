...
- 首页无外部服务请求；成绩海报二维码仅调用随站发布的 `vendor/qrcode-generator.js`（MIT），不会连接第三方域名。离线完整运行需保留 `index.html` 与 `vendor/`。
...
- 分享：结算页生成 1080 × 1350 图片海报，包含本局防篡改感印记、成绩、二维码、七档称号，以及六种按权重抽取的普通到隐藏版式；Web Share 支持时直接分享图片，否则可长按保存。
- 惊喜：十二种洗牌式小意外与 404、555、888、1314、2026 米暗号彩蛋；图鉴只记录正式单人局，测试与多人局不污染存档。
