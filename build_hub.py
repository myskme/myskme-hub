        ...
        {"label": "娱乐", "anchor": "vol-1", "vol": "壹", "era": "第一纪 · 游戏与世界观",
         "epigraph": "剑与星辰，茶与远方 —— 走进狼先生学院的世界。", "icon": "sword", "items": [
            ...
            {"key": "monkey-upstairs", "glyph": "猴", "cat": "game", "rarity": "SSR", "cover": "assets/cover-monkey-upstairs.svg",
             ...
             "desc": "左右两键冲上 100 层 · 全程物理可达的随机楼道 · 九格长期收藏与香蕉商店十八件行头 · 十六个称号、二十款奶茶、十二种建筑立面与调式、八种世界窗景每局轮换，绕口令办随机营业。可暂停、可装到桌面，结算生成带正版二维码的随机成绩海报。",
...
(function(){
  ...
  function mergeVisualDefaults(data){
    var LEGACY_DEFAULT_DESC={
      ...
      'monkey-upstairs':[
        ...
        '左右两键冲上 100 层 · 全程物理可达的随机楼道 · 十六个称号与二十款奶茶图鉴 · 绕口令办与百层倒班随机营业。可随时暂停，可安装到桌面，结算生成带游戏二维码的随机成绩海报。'
      ],
      zimingqi:[
    ...
    // 0815 新增课堂答题器。旧主页存档也要补到学习卷首；只补官方缺失项，
    // 不覆盖王老师改过的作品，也不改变现有自建卡片。
    var learning=data.sections&&data.sections[1];
    var classroomClicker=DEFAULT_ITEM_BY_KEY['classroom-clicker'];
    if(learning&&classroomClicker&&!present['classroom-clicker']){
      learning.items=learning.items||[];
      learning.items.unshift(clone(classroomClicker));
    }
    return data;
  }
  ...
  var ITEM_PATH={
    ...
    'classroom-clicker':'<path d="M4 20V8l8-4 8 4v12"/><path d="M8 20v-7h8v7M9 9h6"/><path d="M6 20h12"/><circle cx="9" cy="16" r="1"/><circle cx="15" cy="16" r="1"/>',
